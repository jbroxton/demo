...
BEGIN
  ...
  PERFORM extensions.pgmq.send('embedding_jobs', job_payload);
...
BEGIN
  ...
  FOR job_record IN 
    SELECT * FROM extensions.pgmq.read('embedding_jobs', 30, max_jobs) -- 30 second visibility timeout
  LOOP
    -- Invoke Edge Function to process this job
    IF public.invoke_edge_function('process-embedding', job_record.message) THEN
      -- Mark job as completed
      PERFORM extensions.pgmq.delete('embedding_jobs', job_record.msg_id);
...
SELECT 
  ...
  oldest_msg_age_sec,
  total_messages
FROM extensions.pgmq.metrics('embedding_jobs');
