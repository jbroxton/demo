-- Fix container networking for Edge Function calls
-- Use Kong gateway for internal container communication
...
DECLARE
  ...
  request_id BIGINT;
BEGIN
  -- FIXED: Use Kong gateway for internal container communication
  -- Kong acts as the API gateway for all Supabase services on port 8000
  ...
  LOOP
    BEGIN
      RAISE LOG 'Processing embedding job % with payload: %', job_record.msg_id, job_record.message;
      
      -- Make HTTP request using net.http_post with correct named parameters
      SELECT INTO request_id net.http_post(
        url := function_url,
        body := job_record.message,
        headers := headers_obj,
