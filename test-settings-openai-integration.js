...
} = require('./src/services/tenant-settings-db');
const { tenantAssistants, isOpenAIConfigured } = require('./src/lib/openai');
...
async function testSettingsSync() {
  ...
  try {
    ...
    const testContent = `# Integration Test Company
...
Unique identifier: ${Math.random().toString(36).substring(7)}`;
    ...
    if (hasTestContent && hasTestPhrase) {
